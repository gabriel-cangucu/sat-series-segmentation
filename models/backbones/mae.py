...
class MAE(nn.Module):
    ...
    def __init__(
        ...
        self.patch_embed = PatchEmbed(img_size, patch_size, in_chans * num_frames, embed_dim)
        ...
        self.decoder_pred = nn.Linear(decoder_embed_dim, patch_size**2 * in_chans * num_frames, bias=True) # decoder to patch
        # --------------------------------------------------------------------------
    ...
    def _init_weights(self, m: nn.Linear | nn.LayerNorm) -> None:
    ...
    def unpatchify(self, x: torch.Tensor) -> torch.Tensor:
        ...
        t = self.num_frames
        c_total = x.shape[-1] // (p ** 2)
        c = c_total // t
        
        x = x.reshape(shape=(x.shape[0], h, w, p, p, c_total))
        x = torch.einsum("nhwpqc -> nchpwq", x)
        imgs = x.reshape(shape=(x.shape[0], c, t, h * p, h * p))
    ...
    def forward(
        ...
        x = rearrange(x, "b (c t) h w -> b c t h w", t=self.num_frames)
