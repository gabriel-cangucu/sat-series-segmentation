...
from models.utils.swin_vit import PatchEmbedding, BasicBlock, PatchExpanding, BasicBlockUp
...
class SwinMAE(nn.Module):
    ...
    """
    def __init__(self, img_size: int = 224, patch_size: int = 4, mask_ratio: float = 0.75,
                 num_frames: int = 1, in_chans: int = 3, decoder_embed_dim=768, norm_pix_loss=False,
                 depths: tuple = (2, 2, 6, 2), embed_dim: int = 96, num_heads: tuple = (3, 6, 12, 24),
                 window_size: int = 3, qkv_bias: bool = True, mlp_ratio: float = 4.,
        ...
        if num_frames > 1:
            raise NotImplementedError("Swin currently only supports 2D images")
